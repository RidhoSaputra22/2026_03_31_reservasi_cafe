<x-modal open="confirmationModalOpen" onClose="closeConfirmationModal()" title="Cek Data Anda" maxWidth="max-w-lg"
    ...
    panelClass="rounded-3xl bg-white p-6 shadow-2xl" overlayClass="bg-black/55" closeLabel="Tutup">
    <div class="space-y-2">
        <p class="text-sm text-gray-500">
            Pastikan detail reservasi sudah benar. Setelah klik tombol buat reservasi, kamu akan diarahkan ke halaman
            pembayaran Midtrans.
        </p>
    </div>

    <div class="mt-5 space-y-3 rounded-2xl bg-gray-50 p-4 text-sm text-gray-600">
        ...
        @if ($downPaymentAmount > 0)
            <div class="rounded-2xl border border-primary/15 bg-primary/5 px-4 py-3 text-xs text-primary">
    ...
    <x-slot:footer>
        <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            <button type="button" @click="closeConfirmationModal()" :disabled="submitting"
                class="rounded-2xl border border-gray-200 px-4 py-3 text-sm font-semibold text-gray-600 transition hover:border-gray-300 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-60">
                Kembali
            </button>
